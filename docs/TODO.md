...
## 진행 현황
- [ ] **B — 통계 커스텀 날짜 범위 (2026-07-10)** — B1~B3 3개 슬라이스(순차 의존). 아래 "통계 커스텀 날짜 범위" 섹션 참조.
...
### Acceptance criteria
- [ ] 일별 추이 집계가 임의의 `from~to` 범위에 대해 하루 단위 포인트를 생성 (기존 `dailyTrend7`/`dailyTrend30` 이분 API는 제거하거나 이 범위 기반 함수로 대체)
- [ ] 기간 대비 배지 로직이 "선택 기간 vs 직전 동일 길이 기간"으로 일반화 (예: 7일 범위면 기존과 동일, 30일이면 직전 30일과 비교)
- [ ] KPI(총 방문/총 클릭/클릭률), 링크별 순위, 요일분포, 유입출처, 캠페인 전부 `from~to` 범위로 필터링된 이벤트 기준으로 재계산
- [ ] events 조회 결과 건수가 limit(10000)과 같으면 `capped: true` 반환
- [ ] 순수 함수 유닛 테스트로 범위 필터링·일반화된 배지 로직·capped 플래그 검증
...
### Acceptance criteria
- [ ] `GET /api/admin/stats?from=YYYY-MM-DD&to=YYYY-MM-DD` — 인증된 관리자만 접근 가능 (기존 `src/proxy.ts` 보호 재사용)
- [ ] 날짜 파라미터 누락·형식 오류·`from > to` 등 잘못된 범위는 400 응답
- [ ] 응답 바디에 B1의 `capped` 플래그 포함
- [ ] Route Handler 통합 테스트 (정상 범위, 잘못된 파라미터, 캡 도달 케이스)
...
### Acceptance criteria
- [ ] 프리셋 6종(오늘/7일/30일/이번달/지난달/전체) 버튼 + 커스텀 시작~종료일 입력
- [ ] 범위 변경 시 B2 API 재조회, 재조회 중 로딩 상태 표시
- [ ] `capped: true` 응답 시 경고 배너 표시
- [ ] 기본 진입 시 최근 7일 범위로 초기 로드
- [ ] 기존 7/30일 토글 완전히 대체 (제거)
- [ ] 컴포넌트 테스트 (프리셋 전환, 커스텀 범위 입력, 캡 배너 표시) + 브라우저 수동 확인
