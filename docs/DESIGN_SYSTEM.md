...
## 9. 관리자 대시보드 상속
- 동일 비비드 블루 팔레트 상속, **데이터 밀도 높은 톤**: 배경 `#F3F6FD`, KPI 카드 흰색 1px 보더 + `--sh-sm`, 차트 색 = 블루 계열(`#1B4DFF`/`#5B82FF`/`#0B2EAE`) + 상태색(good/danger).
- 스타일: Data-Dense Dashboard(KPI 카드 + 막대/라인/파이). 표는 정렬·hover 하이라이트.
- 차트 라이브러리: Recharts 또는 경량 대안 — 3.5단계 리소스 승인에서 확정.
